...
# 可选导入 moviepy，如果未安装则跳过
try:
    from moviepy import VideoFileClip, AudioFileClip
    MOVIEPY_AVAILABLE = True
except ImportError:
    MOVIEPY_AVAILABLE = False
    print("[警告] moviepy 未安装，视频合并功能将使用 ffmpeg。如需安装 moviepy，请运行: pip install moviepy")
        ...
        if os.system("ffmpeg -version") != 0:
            ...
            if MOVIEPY_AVAILABLE:
                ...
                try:
                    ...
                    print(f"使用moviepy合并视频失败: {e}")
                    print("请安装 ffmpeg 或安装 moviepy: pip install moviepy")
                    raise
            else:
                print("ffmpeg 和 moviepy 都未安装，无法合并视频音频")
                print("请安装 ffmpeg (推荐) 或安装 moviepy: pip install moviepy")
                raise ImportError("需要安装 ffmpeg 或 moviepy 来合并视频音频")
        else:
            # 为了防止路径中有空格等ffmpeg不支持字符，使用双引号
            os.system(f'ffmpeg -i "{video_path}" -i "{audio_path}" -c:v copy -c:a aac "{save_path}"')
